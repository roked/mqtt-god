...
public class MqttService extends Service implements MqttTraceHandler {
   ...
   */
    ...
    public Status acknowledgeMessageArrival(String clientHandle, String id) {
        return messageStore.discardArrived(clientHandle, id) ? Status.OK : Status.ERROR;
    }
    ...
    public void onDestroy() {
        for (MqttConnection client : connections.values()) {
            client.disconnect(null, null);  // disconnect immediately
        }
        if (mqttServiceBinder != null) {    // clear down
    ...
    public IBinder onBind(Intent intent) {
        // What we pass back to the Activity on binding - a reference to ourself, and the
        // activityToken we were given when started
    ...
    private class NetworkConnectionIntentReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            // we protect against the phone switching off by requesting a wake lock - we request the
            // minimum possible wake lock - just enough to keep the CPU running until we've finished
    ...
    public boolean isOnline() {
        ...
        if (networkInfo != null && networkInfo.isAvailable()
                && networkInfo.isConnected() && backgroundDataEnabled) {
            return true;
        }
        return false;
    }
