...
public class MqttAndroidClient extends BroadcastReceiver implements IMqttAsyncClient {
    ...
    /**
     * Close the client. Releases all resource associated with the client. After the client has
        ...
        if (mqttService != null) {
            if (clientHandle == null) {
                clientHandle = mqttService.getClient(serverURI, clientId,
                        myContext.getApplicationInfo().packageName, persistence);
            }
     ...
     * </p>
     *
     * @return token used to track and wait for the connect to complete. The token will be passed
     * to the callback methods if a callback is set.
    ...
    public IMqttToken connect(MqttConnectOptions options, Object userContext,
        ...
        connectToken = token;
        /*
         * The actual connection depends on the service, which we start and bind to here, but which
         * we can't actually use until the serviceConnection onServiceConnected() method has run
         * (asynchronously), so the connection itself takes place in the onServiceConnected() method
         */
     ...
     * @return token used to track and wait for disconnect to complete. The token will be passed
     * to the callback methods if a callback is set.
     * @throws MqttException for problems encountered while disconnecting
     * @see #disconnect(long, Object, IMqttActionListener)
     */
     ...
     * @return token used to track and wait for the publish to complete. The token will be passed
     * to any callback that has been set.
     ...
     * @param retained    whether or not this message should be retained by the server.
     * @param userContext optional object used to pass context to the callback. Use null if not required.
     * @param callback    optional listener that will be notified when message delivery has completed
     *                    to the requested quality of service
     * @return token used to track and wait for the publish to complete. The token will be passed
     * to any callback that has been set.
     * @throws MqttPersistenceException when a problem occurs storing the message
     * @throws IllegalArgumentException if value of QoS is not 0, 1 or 2.
    ...
    public IMqttDeliveryToken publish(String topic, byte[] payload, int qos, boolean retained,
        ...
        MqttDeliveryTokenAndroid token = new MqttDeliveryTokenAndroid(this, userContext,
                callback, message);
        String activityToken = storeToken(token);
        IMqttDeliveryToken internalToken = mqttService.publish(clientHandle, topic, payload, qos,
                retained, null, activityToken);
    ...
    public IMqttDeliveryToken publish(String topic, MqttMessage message, Object userContext,
        ...
        String activityToken = storeToken(token);
        IMqttDeliveryToken internalToken = mqttService.publish(clientHandle, topic, message,
                null, activityToken);
        token.setDelegate(internalToken);
        return token;
    }
     ...
     *              quality of service will be received at the published QoS. Messages published at a higher
     *              quality of service will be received using the QoS specified on the subscription.
     * @return token used to track and wait for the subscription to complete. The token will be
     ...
     * @return token used to track and wait for the subscribe to complete. The token will be passed
     * to callback methods if set.
     ...
     * </ul>
     *
     * @param topic       one or more topics to unsubscribe from. Each topic must match one specified
     *                    on an earlier subscription.
     * @param userContext optional object used to pass context to the callback. Use null if not required.
     * @param callback    optional listener that will be notified when unsubscribe has completed
     * @return token used to track and wait for the unsubscribe to complete. The token will be
     * passed to callback methods if set.
     ...
     *
     * @param messageId the messageId received from the MqttMessage (To access this field you need
     *                  to cast {@link MqttMessage} to {@link ParcelableMqttMessage})
    ...
    private void sendAction(Bundle data) {
        ...
        simpleAction(token, data);    // remove on delivery
    }
